...
phase_preflight() {
    ...
    # Accept Arch itself (ID=arch) and Arch derivatives, which identify
    # themselves by ID_LIKE=arch — EndeavourOS is ID="endeavouros",
    # ID_LIKE="arch", so an ID-only check rejects it.
    if [ ! -f /etc/os-release ] || ! grep -qiE '^ID(_LIKE)?=.*\barch' /etc/os-release; then
        err "This script only knows how to set things up on Arch or an Arch-based distro."
        return 1
    fi
    ok "Running on $(. /etc/os-release 2>/dev/null && echo "${PRETTY_NAME:-Arch Linux}")."
